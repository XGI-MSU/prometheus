...
class Data:
    ...
    def __init__(self,
        ...
        self.per_psr_data_dict = build_per_psr_data_dict(self.psrs,
                                                         self.wn_dict,
                                                         self.nfreqs,
                                                         self.psr_dists_dict,
...
def build_per_psr_data_dict(psrs : list[PintPulsar] | list[FeatherPulsar],
    ...
    nfreqs : int
        Number of frequency bins to use in pulsar noise model.
    psr_dists_dict : dict
        Dictionary of pulsar distances, uncertainties, and measurement
        method (e.g. parallax or DM). If None, the pulsar distances
        and uncertainty from 'psrs' is used.
    det_window_ext_factor : float
        Factor by which to increase modeling window (either side)
        for continuous waves to reduce Gibbs phenomena in Fourier
    ...
    with tqdm(range(len(psrs)), desc='building pulsar models') as pbar:
        for i in pbar:
            psr = psrs[i]
            ...
            window_ext = Tspan * det_window_ext_factor
            ...
            if psr_dists_dict is not None:
                pdist = psr_dists_dict[psr.name]
